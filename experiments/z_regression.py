...
def add_subparser(subparsers):
    ...
    parser = subparsers.add_parser("regression-old",
                                   aliases=["reg_old"],
                                   help="regression",
...
def main(dataset, num_hiddens, w_variance, b_variance, activation,
    ...
    # Valid
    train_valid_nngp_kernel = kernel_fn_2(train_x, train_x, "nngp")
    ...
    for alpha, beta in alpha_beta:
        ...
        inverse_k_11 = jnp.linalg.inv(covariance + epsilon_variance * jnp.eye(train_num))
        d_1 = nu + matmul3(train_y.T, inverse_k_11, train_y)

        conditional_kernel = d_1 / conditional_nu * beta / alpha * nngp_covariance_2_valid

        valid_std = jnp.sqrt(jnp.diag(conditional_kernel))

        # Test

        conditional_kernel = d_1 / conditional_nu * beta / alpha * nngp_covariance_2_test
