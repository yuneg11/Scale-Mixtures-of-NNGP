...
def main(dataset, num_hiddens, w_variance, b_variance, activation, alpha, beta,
         epsilon_log_variance, seed, sample_num, model, **kwargs):
    ...
    for alpha in alpha_set:
        for beta in beta_set:
            ...
            exact_dict[f"{alpha} - {beta}"] = (round(float(nll_valid_invgamma), 6), round(float(nll_test_invgamma), 6))
    ...
    for alpha in alpha_set:
        for beta in beta_set:
            sample_q = stats.invgamma.rvs(a=alpha, loc=0., scale=beta, size=sample_num, random_state=101)
            ...
            prob_prior = stats.invgamma.pdf(sample_q, a=alpha, loc=0., scale=beta).reshape(-1, 1)
            prob_q = stats.invgamma.pdf(sample_q, a=alpha, loc=0., scale=beta).reshape(-1, 1)
            ...
            mc_dict[f"{alpha} - {beta}"] = (round(float(nll_valid_burr12), 6), round(float(nll_test_burr12), 6))


    for alpha in alpha_set:
        for beta in beta_set:
            key = f"{alpha} - {beta}"
            exact_valid, exact_test = exact_dict[key]
            mc_valid, mc_test = mc_dict[key]

            utils.print_yaml(type="invgamma", **base_args,
                            **{"alpha": alpha, "beta": beta,
                                "exact": (exact_valid, exact_test),
                                "mc": (mc_valid, mc_test),
                                "valid-diff": round(float(exact_valid - mc_valid), 6),
                                "test-diff": round(float(exact_test - mc_test), 6)})
